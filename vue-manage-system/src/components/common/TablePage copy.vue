    ...
    <div class="table">
        ...
        <div class="container">
            <div class="handle-box">
                ...
                <div class="query-box" v-for="item in queryCondition" :key="item.model">
                    <span>{{item.name}}：</span>
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange">
                ...
                <el-table-column v-for="item in columns" :key="item.prop" :prop="item.prop" :label="item.label" :sortable="item.sortable"></el-table-column>
        ...
        <el-dialog :title="getDialogTitle" :visible.sync="editVisible" width="30%">
            <el-form ref="form" :model="form" label-width="50px" :rules="rules">
                <el-form-item  v-for="item in formCondition" :key="item.prop" :label="item.label" :prop="item.prop">
                     <el-input v-if="item.type === 'input'" v-model="form[item.prop]" :placeholder="item.placeholder" :disabled="!isEdit"></el-input>
                     <el-select v-else-if="item.type === 'select'" :placeholder="item.placeholder" v-model="form[item.prop]">
                        <el-option
                        v-for="list in item.list"
                        :key="list[item.key]"
                        :label="list[item.label]"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //查询数据
            queryData(){
                crudData(this.url.query,this.query).then((res) => {
            ...
            editData(formName) {
                console.log(this.form)
                 this.$refs[formName].validate((valid) => {
                    if (valid) {
                         let url = this.edit === "add" ? this.url.add : this.url.update;
            ...
            deleteData() {
                ...
                crudData(this.url.delete,{id:ids},'post').then((res) => {
           ...
           //编辑弹出框
            handleEdit(row,type) {
                ...
                this.edit = type;
                let formCondition = this.formCondition;
                let len =  formCondition.length;
                let formObj = {
                    id:row.id
                }
                for (let i = 0; i < len; i++) {
                   let prop = formCondition[i].prop;
                   formObj[prop] = row[prop];
                }
                formObj.price = '122';
                formObj.density = '0.999';
                formObj.endPrice = '1';
                this.form = formObj;
                console.log(this.form)
